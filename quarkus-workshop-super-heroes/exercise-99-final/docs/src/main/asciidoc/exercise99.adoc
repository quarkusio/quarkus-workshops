...
In <<exercise99-listing-01>> the `BookService` fires an event (`bookAddedEvent`) each time a book is created.
...
[[exercise99-listing-01]]
.The BookService Fires an Event Each Time a Book Is Created
[source]
----
include::{github-raw}/exercise-99-final/hero-api/src/main/java/io/quarkus/workshop/superheroes/hero/Hero.java[]
----
