...
ifdef::use-windows[TIP: If you would rather not install anything, Windows PowerShell can pretty-print JSON out of the box: `curl http://localhost:8083/api/heroes | ConvertFrom-Json | ConvertTo-Json`.]
