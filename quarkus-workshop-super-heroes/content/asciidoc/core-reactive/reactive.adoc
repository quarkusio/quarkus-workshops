...
plantuml::plantuml/3-reactive-physical-architecture.puml[align=center,width=300,theme="quarkus from {plantuml-theme-dir}"]
...
The easiest way to create this new Quarkus project is to use the Quarkus CLI.
Open a terminal and run the following command under the `quarkus-workshop-super-heroes/super-heroes` directory
...
[source,shell,subs="attributes+"]
----
quarkus create app io.quarkus.workshop.superheroes:rest-heroes \
    --extension="rest-jackson,quarkus-hibernate-validator,quarkus-smallrye-openapi,quarkus-hibernate-reactive-panache,quarkus-reactive-pg-client" \
    --java={jdk-version} \
    --gradle-kotlin-dsl
----
--
...
[source,kotlin]
----
include("super-heroes:rest-heroes")
----
--
endif::use-gradle[]
...
ifdef::use-gradle[]
First, make sure the tests pass by executing the command `./gradlew test` (or from your IDE).
endif::use-gradle[]
--
...
ifdef::use-gradle[]
Use `./gradlew quarkusDev` to start it.
endif::use-gradle[]
...
ifdef::use-gradle[]
Then, build the application using: `./gradlew build`, and run the application using `java -jar build/quarkus-app/quarkus-run.jar`.
endif::use-gradle[]
