...
{#diagram asciidoc=true language="plantuml" align=center,width=300,theme="quarkus from {plantuml-theme-dir}" alt="Impossible trident" width=500 height=500 diagramOutputFormat="png"}

include::{plantdir}/plantuml/5b-ai-narration-microservice.puml[]
...
As before, the easiest way to create this new Quarkus project is to use the Quarkus CLI (you can also go to https://code.quarkus.io if you prefer).
...
[source,shell,subs="attributes+"]
----
quarkus create app io.quarkus.workshop.superheroes:rest-narration \
    --extension="rest-jackson,quarkus-smallrye-openapi,quarkus-smallrye-fault-tolerance" \
    --java={jdk-version} \
    --gradle-kotlin-dsl
----
****
...
--
Once the project is created, you need to add the Quarkus LangChain4j OpenAI dependency.
Add the following to the `rest-narration/build.gradle.kts` file:

[source,kotlin,subs="attributes+"]
----
val quarkusLangchain4jVersion = "{quarkus-langchain4j-version}"
...
* `@UserMessage` — provides the user prompt template.
Template variables like `{fight.winnerName}` are resolved from the method parameters.
...
ifdef::use-gradle[]
Make sure the tests pass by executing the command `./gradlew test` (or from your IDE).
endif::use-gradle[]
--
...
ifdef::use-gradle[]
Make sure you have set the `OPENAI_API_KEY` environment variable, then use `./gradlew quarkusDev` to start it.
endif::use-gradle[]
...
ifdef::use-gradle[]
Now execute `./gradlew test` and make sure that all the tests pass.
endif::use-gradle[]
