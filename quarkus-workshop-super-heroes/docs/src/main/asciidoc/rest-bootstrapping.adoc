...
It's a very simple REST endpoint, returning "Hello RESTEasy" to requests on `/api/heroes`.
...
[INFO] Building rest-hero 1.0.0-SNAPSHOT
[INFO] --------------------------------[ jar ]---------------------------------
[INFO]
[INFO] --- quarkus-maven-plugin:2.2.3.Final:dev (default-cli) @ rest-hero ---
...
2020-01-28 15:09:02,111 INFO  [io.quarkus] (main) rest-hero 1.0-SNAPSHOT (running on Quarkus 2.2.3.Final) started in 2.161s. Listening on: http://0.0.0.0:8080
...
* `rest-hero-1.0.0-SNAPSHOT.jar` : containing just the classes and resources of the projects, it's the regular artifact produced by the Maven build;
* `/quarkus-app/quarkus-run.jar` : executable jar that starts the application under `quarkus-app`.
* `/quarkus-app/app/rest-hero-1.0.0-SNAPSHOT.jar` :
Classes and resources of the projects that need the dependencies located into the `target/quarkus-app/lib` directory.
