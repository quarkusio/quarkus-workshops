...
The Maven archetype generates the following `rest-villain` sub-directory:
...
Use: `./mvnw quarkus:dev` in the `rest-villain` directory:
...
* `rest-villain-1.0-SNAPSHOT.jar`: containing just the classes and resources of the projects, it's the regular artifact produced by the Maven build;
