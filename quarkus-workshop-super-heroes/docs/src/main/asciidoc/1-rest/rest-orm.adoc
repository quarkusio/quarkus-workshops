...
* Uses Hibernate with Panache - so need the extension providing it
...
List<Villain> heroes = Villain.listAll();
...
Notice that both methods that persist and update a hero, pass a `Villain` object as a parameter.
...
ArC handles injected at build time.
You can use field injection and inject the `VillainService` using:
...
So, we need to configre the database access in the `src/main/resources/application.properties` file,
