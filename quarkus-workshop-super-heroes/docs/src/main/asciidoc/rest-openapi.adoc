...
[source,shell]
----
$ curl $(oc get route -n USERNAME-codeready | grep 8083 | awk '{ print $2 }')/openapi
...
If you go back to the `$URL/openapi` endpoint you will see the following OpenAPI contract:
