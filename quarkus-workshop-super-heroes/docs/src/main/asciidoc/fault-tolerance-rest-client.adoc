...
Having this configuration means that all requests performed using `HeroService` will use your specific rest-fight service exposed by the `USERNAME-codeready` project, http://localhost:8083 by default, as the base URL.
...
In order to make it work, you need to set up the `WKSPC_SERVICE_URL` env var. You can do it running the following command:

[source, shell]
--
$ export WKSPC_SERVICE_URL="http://$(oc get svc -n USERNAME-codeready | grep 8083 |  awk '{ print $1 }')"
--
