...
In the runtime module, creates the `src/main/java` directory, and, in this directory, the `io.quarkus.workshop.superheroes.version.runtime.VersionRecorder` class with the following content:
...
Open the `ExtensionVersionProcessor` class, and update the content to be:
...
The code is straightforward: `enabled` is `true`, calls the recorder with the version.
