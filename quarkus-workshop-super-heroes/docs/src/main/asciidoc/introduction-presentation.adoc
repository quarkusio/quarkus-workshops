...
This workshop will make use of the following software, tools, frameworks:

* Code Ready Workspaces as IDE.
* JDK 8
* Maven 3.6.x
* cURL (or any other command line HTTP client)
* Node JS (optional, only if you are in a _frontend_ mood)

Your environment is remote and can be accessed via Code Ready Workspaces (CRW) through your local browser, you just need to configure some elements.
