...
ifdef::use-gradle[]
This will add the following dependency in the `build.gradle.kts` file:
endif::use-gradle[]
...
ifdef::use-gradle[]
Add the extended Quarkus Mockito support to the `fight` service's `build.gradle.kts`:
endif::use-gradle[]
...
ifdef::use-gradle[]
Time to play.
Start the Hero, Villain and Fight microservices as well as the user interface (using `./gradlew quarkusDev` on each project) and select random fighters.
endif::use-gradle[]
