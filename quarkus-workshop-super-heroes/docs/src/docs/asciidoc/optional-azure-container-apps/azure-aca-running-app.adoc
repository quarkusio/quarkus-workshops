...
In this chapter we will create the needed infrastructure in Azure Container Apps (database, etc.) and then deploy the containers so we can execute our application.
endif::use-messaging[]
ifdef::use-messaging[]
...
[source,shell,indent=0]
----
include::{projectdir}/infrastructure/azure-deploy-aca.sh[tags=adocAppNarrationURL]
----
...
ifndef::use-messaging[]
The Fight microservice invokes the Heroes and Villains microservices and stores the fights into a MongoDB database.
We need to configure the Postgres database.
...
[source,shell,indent=0]
----
include::{projectdir}/infrastructure/azure-deploy-aca.sh[tags=adocCreateAppFight;!adocKafkaConfig]
----
endif::use-messaging[]
ifdef::use-messaging[]
[source,shell,indent=0]
