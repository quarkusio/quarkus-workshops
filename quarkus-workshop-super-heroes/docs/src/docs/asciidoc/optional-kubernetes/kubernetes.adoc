...
Here, we are going to use the PostgreSQL Operator by Dev4Ddevs.com.
endif::use-messaging[]
ifdef::use-messaging[]
...
ifdef::use-gradle[]
Alternatively, you can add the following dependency to the `build.gradle.kts` file directly:
endif::use-gradle[]
...
[source, shell]
----
./gradlew clean build -Dquarkus.kubernetes.deploy=true -x test
----
--
endif::use-gradle[]
