...
ifdef::use-no-quarkus-cli[]
:app-name: rest-villains
include::../core-introduction/creating-app-without-quarkus-cli.adoc[]
endif::use-no-quarkus-cli[]
endif::use-gradle[]
...
ifdef::use-no-quarkus-cli[]
Instead of creating the files by hand, you can use https://code.quarkus.io and select the `rest-jackson` extension (make sure to select Gradle as the build tool) if that site is reachable from your environment.
endif::use-no-quarkus-cli[]
endif::use-gradle[]
====
...
ifdef::use-no-quarkus-cli[]
If you scaffolded your project by hand, don't worry about the missing `Dockerfile`s and Java source files.
We will fill them in as we need them.
endif::use-no-quarkus-cli[]
...
ifdef::use-no-quarkus-cli[]
A Quarkus Gradle project has the following `rest-villains` sub-directory:
endif::use-no-quarkus-cli[]
...
* `build/quarkus-app/` : this directory uses the _fast jar_ packaging.
It contains an executable jar (`quarkus-run.jar`), and all the dependencies (structured into `app`, `lib` and `quarkus`).
