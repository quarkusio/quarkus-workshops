...
[source,shell]
----
./gradlew addExtension --extensions="jdbc-postgresql,hibernate-orm-panache,hibernate-validator"
----
--
endif::use-gradle[]
...
[.tab-gradle]
--
This will add the following dependencies in the `build.gradle.kts` file:
...
ifdef::use-gradle[]
From now on, you can choose to either edit your `build.gradle.kts` directly or use the `addExtension` command.
endif::use-gradle[]
...
The same approach works for other Dev Services (for example `quarkus.redis.devservices.image-name`); each extension exposes its own `image-name` property.
endif::use-messaging[]
ifdef::use-messaging[]
...
[.tab-gradle]
--
As previously, you will get your application in `build/quarkus-app`, run it using:
