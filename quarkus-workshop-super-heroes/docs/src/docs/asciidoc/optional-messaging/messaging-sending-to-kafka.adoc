...
[source,shell]
----
./gradlew addExtension --extensions="io.quarkus:quarkus-smallrye-reactive-messaging-kafka"
----
--
endif::use-gradle[]
...
ifdef::use-gradle[]
If not yet started, start the microservice using `./gradlew quarkusDev`.
endif::use-gradle[]
