...
If you are doing the optional messaging section, also start the Kafka infrastructure:
...
If you started the Kafka infrastructure, also shut it down:
...
In our context, we are going to use PostgreSQL as the database and Prometheus as the monitoring tool.
endif::use-messaging[]
ifdef::use-messaging[]
