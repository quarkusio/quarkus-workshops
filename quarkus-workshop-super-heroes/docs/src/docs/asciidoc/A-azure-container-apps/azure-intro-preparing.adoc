...
[source,shell]
----
ACR_PULL_SERVICE_PRINCIPAL_NAME=acr-pull-$UNIQUE_IDENTIFIER

ACR_ID=$(az acr show --name $REGISTRY --resource-group $RESOURCE_GROUP --query id --output tsv)

ACR_PULL_SERVICE_PRINCIPAL_PASSWORD=$(az ad sp create-for-rbac \
...
ACR_PULL_SERVICE_PRINCIPAL_ID=$(az ad sp list \
