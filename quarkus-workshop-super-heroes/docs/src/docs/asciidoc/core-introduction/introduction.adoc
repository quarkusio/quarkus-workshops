...
If you go to the https://quarkus.io[Quarkus web site], you'll read that Quarkus is "_Supersonic Subatomic Java_."
...
ifndef::use-kubernetes[]
Quarkus applications are tailored for the Cloud and containers.
...
You need JDK {jdk-version} and Docker on your machine.
The Quarkus CLI makes creating projects much easier, but these instructions assume it is not available in your (locked-down) environment.
endif::use-no-quarkus-cli[]
endif::use-gradle[]
...
* How to create managed services in Azure (Postgres database)
endif::use-messaging[]
ifdef::use-messaging[]
