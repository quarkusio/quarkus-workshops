...
plantuml::plantuml/0-introduction-physical-architecture.puml[align=center,theme="quarkus from {plantuml-theme-dir}"]
...
in this section, you will develop a microservice architecture by creating several projects, write some Java code, add JPA entities, JAX-RS REST endpoints, write some tests, use a React web application, and all that on Quarkus
ifdef::use-ai[]
...
ifdef::use-gradle[]
The workshop supports both Maven and Gradle as build tools — you can choose either one.
There is no need to install them: the workshop scaffolding includes both a Maven wrapper (`mvnw`) and a Gradle wrapper (`gradlew`).
If you'd prefer to install your own, see the instructions for xref:appendix-installing-maven[Maven] or xref:appendix-installing-gradle[Gradle] at the end of the workshop.
endif::use-gradle[]
endif::use-maven[]
ifndef::use-maven[]
ifdef::use-gradle[]
We will also be using Gradle, but there is no need to install it globally.
The workshop scaffolding includes a Gradle wrapper, `gradlew`.
If you'd prefer to install your own Gradle and don't already have it, xref:appendix-installing-gradle[see the instructions at the end of the workshop].
endif::use-gradle[]
endif::use-maven[]
