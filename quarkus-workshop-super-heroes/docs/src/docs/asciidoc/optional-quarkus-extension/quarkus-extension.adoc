...
plantuml::plantuml/8-extension-architecture.puml[align=center,theme="quarkus from {plantuml-theme-dir}"]
...
----
./mvnw io.quarkus.platform:quarkus-maven-plugin:{quarkus-version}:create-extension -N \
...
The second parameter of the `recordVersion` method is the `VersionConfig` which provides access to the user configuration we created in the previous step.
...
[.tab-gradle]
--
Go back to the _villain_ microservice, and add the following dependency to the `build.gradle` file:

[source,groovy,indent=0]
----
implementation 'io.quarkus.workshop.super-heroes:extension-version:1.0.0-SNAPSHOT'
----
--
endif::use-gradle[]
