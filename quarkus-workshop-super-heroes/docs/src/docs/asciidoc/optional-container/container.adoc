...
When we bootstrapped our microservices, we added a set of `Dockerfiles` under `src/main/docker`:
endif::use-no-quarkus-cli[]
endif::use-gradle[]
...
ifndef::use-messaging[]
It declares all the needed infrastructure (databases) as well as our microservices.
endif::use-messaging[]
ifdef::use-messaging[]
