...
[source,shell]
----
./gradlew -p rest-fights addExtension --extensions="jib"
./gradlew -p rest-heroes addExtension --extensions="jib"
./gradlew -p rest-villains addExtension --extensions="jib"
./gradlew -p ui-super-heroes addExtension --extensions="jib"
ifdef::use-messaging[]
./gradlew -p event-statistics addExtension --extensions="jib"
endif::use-messaging[]
ifdef::use-ai[]
./gradlew -p rest-narration addExtension --extensions="jib"
endif::use-ai[]
----
--
endif::use-gradle[]
...
[.tab-gradle]
--
It adds the following dependency to the `build.gradle.kts` files:
...
ifdef::use-gradle[]
Then, from the root `super-heroes` directory, execute the following commands to build the applications and create the container image (add `-x test` if you want to skip the tests):
endif::use-gradle[]
--
...
[source, shell]
----
./gradlew -p rest-fights clean build -Dquarkus.container-image.build=true
./gradlew -p rest-heroes clean build -Dquarkus.container-image.build=true
./gradlew -p rest-villains clean build -Dquarkus.container-image.build=true
./gradlew -p ui-super-heroes clean build -Dquarkus.container-image.build=true
ifdef::use-messaging[]
./gradlew -p event-statistics clean build -Dquarkus.container-image.build=true
endif::use-messaging[]
ifdef::use-ai[]
./gradlew -p rest-narration clean build -Dquarkus.container-image.build=true
endif::use-ai[]
----
--
endif::use-gradle[]
