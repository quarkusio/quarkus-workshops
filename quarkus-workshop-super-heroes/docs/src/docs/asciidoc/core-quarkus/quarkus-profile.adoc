...
ifdef::use-gradle[]
* `dev` - Activated when in development mode (i.e. `quarkusDev`)
endif::use-gradle[]
...
ifdef::use-gradle[]
Now, you will get the `dev` profile enabled if you start your application in dev mode with `./gradlew quarkusDev`.
endif::use-gradle[]
...
ifdef::use-gradle[]
package your application with `./gradlew build`, and start it with `java -Dquarkus.profile=foo -jar build/quarkus-app/quarkus-run.jar`.
endif::use-gradle[]
