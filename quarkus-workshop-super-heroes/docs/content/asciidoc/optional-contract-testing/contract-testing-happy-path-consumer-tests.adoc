...
ifdef::use-gradle[]
Add the Pact Quarkus extension to the `build.gradle.kts` of the Fights service:
endif::use-gradle[]
--
...
----
testImplementation("io.quarkiverse.pact:quarkus-pact-consumer:{quarkus-pact-version}")
----
--
endif::use-gradle[]
...
ifdef::use-gradle[]
Run the build, using `./gradlew test` or `./gradlew quarkusDev`.
endif::use-gradle[]
