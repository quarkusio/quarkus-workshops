...
Antonio Goncalves; Clement Escoffier; Holly Cummins; Georgios Andrianakis; Emmanuel Bernard; George Gastaldi; Martin Stefanko; Eric Deandrea; Aurea Munoz Hernandez :doctype: book :source-highlighter: coderay :source-language: java
...
// projectdir, plantDir and plantuml-theme-dir are set in config/application.properties. They have
// to be, because every .adoc here is also rendered as a standalone Roq page and so cannot rely on
// attributes defined in spine.adoc. Attributes passed through the API are locked, so redefining
// them here would have no effect anyway.
// Conditional includes. The old Maven build swapped this for a generated per-variant options.adoc
// to produce tailored docs; that step is not ported to the Roq build yet, so the defaults are
// included directly. Referencing it through an attribute is not an option here, because Roq's
// build-time header scanner resolves include:: without the attributes from application.properties.
include::_default-options.adoc[]
...
// Convert from buildtool named attribute to booleans
ifeval::["{buildtool}"=="maven"]
:use-maven:
endif::[]
ifeval::["{buildtool}"=="gradle"]
:use-gradle:
endif::[]
// Fallback to Maven if buildtool isn't set
ifndef::buildtool[]
:use-maven:
endif::[]
// Derive no-quarkus-cli from locked-down. For now it is always on in locked-down
// environments; it can be promoted to its own flag later if needed.
